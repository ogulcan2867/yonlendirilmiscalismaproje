	...
	var Button = function () {
		...
		this.update = function ($container, styleInfo) {
			...
			if (styleInfo.image) {
				...
				var $title = $container.find('.arg-inpg input[data-txh="title"]');
				$title.val($img.attr('title') !== undefined ? $img.attr('title') : "");
				$("body").on('change', 'input[data-txh="title"]', function (e) {
					e.preventDefault();
					$img.attr('title', $(this).val());
					$($img.parents('.note-editor').data("holder")).trigger('summernote.change');
				});
		...
		this.showImageDialog = function ($editable, $dialog) {
			return $.Deferred(function (deferred) {
				...
				$imageDialog.one('shown.bs.modal', function () {
					...
					var $progressbar = $imageDialog.find('.progress:eq(0)');
					$progressbar.addClass('hidden');
					$progressbar.find('.progress-bar').css({width: 0});
					$imageInput.replaceWith($imageInput.clone()
							.on('change', function () {
								var formDatax = new FormData();
								formDatax.append("userfile", this.files[0]);
								formDatax.append('action', 'fileupload');
								$.ajax({
										...
										if (myXhr.upload) { // Check if upload property exists
											myXhr.upload.addEventListener('progress', function (e) {
												...
												if (e.lengthComputable) {
													$progressbar.find('.progress-bar').css({width: (e.loaded / e.total * 100) + "%"});
												}
	...
	var Renderer = function () {
		...
		var tplPopovers = function (lang, options) {
			...
			var tplImagePopover = function () {
				...
				var titleInput = '<div class="arg-inpg" style="width: 240px;margin-top: 5px; margin-right: 5px;"><div class="input-group"><span class="input-group-addon">' + lang.image.imageTitle + '</span><input type="text" data-txh="title" class="form-control"></div></div>';


				var content = titleInput + (options.disableResizeImage ? '' : '<div class="btn-group">' + fullButton + halfButton + quarterButton + '</div>') +
		...
		var tplDialogInfo = {
			image: function (lang, options) {
				...
				var body = '<div class="form-group row note-group-select-from-files">' +
						...
						imageLimitation +
						'<div class="progress hidden" style="margin-top: 15px; padding: 0;">' +
